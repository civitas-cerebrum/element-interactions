...
# ---------------------------------------------------------------------------
section "ledger-gate: hole-A — ledger-absent requires no Phase-3+ specs"
# Rule 5 (revised): silent-allow on missing ledger ONLY when no spec
# files exist. Once tests/e2e/<journey>.spec.ts has landed (Phase 3+),
# the ledger is mandatory.
clear_ledger
rm -rf "$TMP_REPO/tests/e2e"
mkdir -p "$TMP_REPO/tests/e2e/docs"
# No specs yet → silent allow (brand-new run, Phase 1 may start).
assert_allow "$H" "$(payload tool_name=Agent description='phase1-scaffold' prompt='Lay down config.' cwd="$TMP_REPO")" \
  "no ledger + no specs → silent allow (genuine fresh run)"
assert_allow "$H" "$(payload tool_name=Agent description='phase2-groundwork' prompt='Author app-context.' cwd="$TMP_REPO")" \
  "no ledger + no specs + phase2 dispatch → silent allow"

# Specs exist but no ledger → DENY (exploit window closed).
mkdir -p "$TMP_REPO/tests/e2e"
printf 'import {test} from "@playwright/test";\ntest("happy", async ({page}) => {});\n' \
  > "$TMP_REPO/tests/e2e/j-happy.spec.ts"
assert_deny "$H" "$(payload tool_name=Agent description='phase4-cycle-1-section-auth' prompt='Map auth section.' cwd="$TMP_REPO")" \
  "spec file exists + no ledger + phase4 dispatch → DENY" "Onboarding ledger missing"
assert_deny "$H" "$(payload tool_name=Agent description='composer-j-bad-login' prompt='Compose. composer.schema.json' cwd="$TMP_REPO")" \
  "spec file exists + no ledger + composer dispatch → DENY" "Phase-3+ deliverables exist"
...
# Cleanup spec & re-create directory layout for downstream tests.
rm -rf "$TMP_REPO/tests/e2e"
mkdir -p "$TMP_REPO/tests/e2e/docs"
