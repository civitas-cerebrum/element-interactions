...
- **Reviewer returns (§2.4):** the top-level `status:` is one of `greenlight`, `greenlight-with-notes`, or `improvements-needed`. Finding blocks (when present under `missing-scenarios:`, `craft-issues:`, or `verification-misses:` sub-lists) match `^ {2}- \*\*[a-z0-9-]+-\d+-\d+-R-\d+\*\* \[(?:must-fix|nice-to-have)\]`. A `greenlight` return has a `summary:` line and no finding blocks; `greenlight-with-notes` has only `nice-to-have` findings; `improvements-needed` has at least one `must-fix` finding. The Stage A regex in the previous bullet does NOT apply to reviewer returns.
