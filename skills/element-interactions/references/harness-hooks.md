...
- **[onboarding-ledger-gate](../../../hooks/onboarding-ledger-gate.sh)** — `PreToolUse:Agent`. Pipeline state-machine enforcement. Reads `tests/e2e/docs/onboarding-status.json` and denies (a) non-`workflow-reviewer-*` Agent dispatches at any transition point where the prior phase's `reviewerVerdict` is still `pending`; (b) out-of-order phase / pass / cycle dispatches (e.g. `phase4-*` while `currentPhase = 2`, `composer-j-…-2` while `pass-1.reviewerVerdict != approved`, `phase4-cycle-2-section-*` while `cycle-1.reviewerVerdict != approved`); (c) any Agent dispatch when the ledger is absent AND at least one `tests/e2e/*.spec.ts` file exists (Phase 3+ deliverables present without a ledger means the state-machine gate was being bypassed wholesale). `workflow-reviewer-*` dispatches always allowed. Ledger-absent silent-allow only when no spec files yet exist (genuine pre-Phase-3 fresh run); malformed ledger still silent-allows (the write-gate owns integrity). [escape hatch: no]
