...
| 4 — Journey mapping | journey-mapping Phases 1–5 | (a) `tests/e2e/docs/journey-map.md` exists with `<!-- journey-mapping:generated -->` on line 1. (b) Coverage-checkpoint signature present (Phase-5 marker). (c) Roster non-empty; matches discovered journeys. (d) Sub-journeys (`sj-<slug>`) properly cross-referenced from journeys. (e) Phase-4 commit landed. (f) `tests/e2e/docs/.phase4-cycle-state.json` exists with `convergence-status` ∈ {`converged`, `hard-cap-reached`}; ≥3 cycles ran (3-cycle floor); ≤5 cycles ran (5-cycle ceiling); each cycle has a `dispatched-sections[]` AND `returned-sections[]` array; `author-dispatched: true`. (g) Every section in the union of `cycles.<N>.returned-sections` is referenced in at least one journey block in `journey-map.md` OR listed under `## Gated Areas (Not Mapped)`. (h) The journey map's frontmatter contains a `**Mapping completeness:**` line consistent with `convergence-status`. |
