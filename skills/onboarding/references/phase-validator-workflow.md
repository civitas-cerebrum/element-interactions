...
| 3 — Happy path | element-interactions Stages 1–4 | (a) `tests/e2e/happy-path.spec.ts` exists. (b) Spec passes 3× green via `npx playwright test happy-path.spec.ts` (verify via the suite-gate window). (c) Stage 4a optimization markers in spec (e.g., `beforeEach(resetState)` per §1, `dismissBanners` per §5 if banners present). (d) Stage 4b api-reference compliance: spec uses Steps API, no raw `page.click(...)` etc. (e) Phase-3 commit landed. (f) `tests/e2e/docs/.discovery-draft.json` exists with `"discovery-draft-version": 1` sentinel; `sections-inferred[]` non-empty; `handover-to-phase4.cycle-1-targets[]` non-empty. (Schema in `../element-interactions/references/autonomous-mode-callers.md`.) |
| 4 — Journey mapping | journey-mapping Phases 1–5 | (a) `tests/e2e/docs/journey-map.md` exists with `<!-- journey-mapping:generated -->` on line 1. (b) Coverage-checkpoint signature present (Phase-5 marker). (c) Roster non-empty; matches discovered journeys. (d) Sub-journeys (`sj-<slug>`) properly cross-referenced — for every journey's `Sub-journey refs: [sj-A, sj-B]` there is a matching `Used by: [..., j-this, ...]` on each named sub-journey, AND every `Used by: [j-X]` entry on a sub-journey has a matching `Sub-journey refs:` line on the named journey, AND no orphan sub-journeys (`Used by: []`), AND no journey references an undefined `sj-<slug>`. (e) Phase-4 commit landed. (f) `tests/e2e/docs/.phase4-cycle-state.json` exists with `convergence-status` ∈ {`converged`, `hard-cap-reached`}; ≥2 cycles ran AND at least one cycle has `kind: "edge-probe"`; ≤5 cycles ran (or ≤10 with `.claude/phase4-extended-cycles-authorized` sentinel); cycles are contiguous 1..N (no gaps); each cycle has a `dispatched-sections[]` and `returned-sections[]` array of equal length; `author-dispatched: true`; `author-attempts` ≤ 3. (g) `## Section → Journey Map` table present and well-formed. Every section in the union of `cycles.<N>.returned-sections` appears as a row in the table (via canonical ID OR a `(novel)` annotation) OR is listed under `## Gated Areas (Not Mapped)`. The validator does NOT use a substring grep — it parses the table and verifies named coverage. (h) The journey map's frontmatter contains a `**Mapping completeness:**` line consistent with the state-file's `convergence-status`. (i) **Structural-smell prevention.** No journey block matches any of these anti-patterns: (i.1) journey title literally contains "mobile" or "viewport" AND another journey block with a similar non-mobile-prefixed title exists (the mobile flow should be a `Mobile: yes` state variation on the parent); (i.2) journey block has fewer than 3 entries in its `Steps:` list; (i.3) journey block's `Steps:` list is a strict subset of another journey's `Steps:` differing only by a viewport directive or empty-state assertion (the smaller block should be a `State variations:` entry); (i.4) journey block exists with `Steps:` consisting only of "load page" + "assert empty/error/not-found state" (this is a state variation, not a journey). Findings in (i) are `pv-4-structural-smell-NN` and downgrade the validator return to `improvements-needed`. |
