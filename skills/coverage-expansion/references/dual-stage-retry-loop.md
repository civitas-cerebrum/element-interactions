...
| "The journey was greenlit last pass with no map delta — skip the whole A↔B for this pass" | Every journey gets both stages every pass, full stop. The trigger-gated re-pass mechanism (#164.1) handles the "no map delta" case at the pass level, not by skipping individual journeys. |
...
| "The hybrid model table says Sonnet for per-journey reviewers, so I'll dispatch Stage B on Sonnet" | No longer true. Per the post-revision §"Hybrid model selection" in [`coverage-expansion/SKILL.md`](../SKILL.md), per-journey Stage B reviewers (across all passes 1–5) are Opus, and the Stage B batch reviewer is Opus. Issue #164's Sonnet/Opus parity was a single-cycle observation on adversarial probe categories — the policy nonetheless keeps Pass 4 probes on Opus (regression-layer input), and review judgement on Opus is independent of any probe-side parity claim. Generalising a single empirical data point to "Sonnet everywhere" is the Sonnet-everywhere drift pattern (see [`anti-rationalizations.md` §"Sonnet-everywhere drift"](anti-rationalizations.md)). Dispatch Stage B on Opus. |
