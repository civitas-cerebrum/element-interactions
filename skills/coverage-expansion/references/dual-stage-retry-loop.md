...
| "The hybrid model table says Sonnet for per-journey reviewers, so I'll dispatch Stage B on Sonnet" | No longer true. Per the post-revision §"Hybrid model selection" in [`coverage-expansion/SKILL.md`](../SKILL.md), per-journey Stage B reviewers (across all passes 1–5) are Opus, and the Stage B batch reviewer is Opus. Issue #164's Sonnet/Opus parity covers Pass 4 Stage A adversarial probes, not review judgement; generalising probe-parity to review-parity is the Sonnet-everywhere drift pattern (see [`anti-rationalizations.md` §"Sonnet-everywhere drift"](anti-rationalizations.md)). Dispatch Stage B on Opus. |
