...
1. Prepare an isolated context window and an isolated Playwright MCP browser instance (same rules as compositional-pass subagents — on Claude Code this is automatic when dispatched via the Agent tool with `mcp__plugin_playwright_playwright__*` in scope; parallel subagents never share a browser).
