...
- **State-file lifecycle.** The state file is a resume marker, not a run log. On **successful completion of all five passes + cleanup**, the orchestrator MUST delete `tests/e2e/docs/coverage-expansion-state.json` as part of the cleanup commit — otherwise the next invocation will mistake a completed run for a resume. On a **fresh invocation**, if the state file is present the orchestrator treats the run as a resume and verifies commit-existence per the previous bullet; it does NOT start from scratch silently. If the file exists but references a journey-map or commit graph that no longer matches reality (e.g., the branch was rebased, or journey IDs changed), the orchestrator stops and reports the conflict to the caller rather than guessing. The conflict-report message MUST name both the specific mismatch (e.g., "state file references commit `abc123` which is not present on the current branch; journey IDs `j-foo` and `j-bar` in state are not in the current `journey-map.md`") AND the user-authorised remediation: invoke with `args: "force-restart"` to delete the state file and start fresh from Pass 1. The orchestrator only deletes the state file when invoked with `force-restart` (or as part of the successful-cleanup commit); silent `rm` by the orchestrator is forbidden and a manual `rm` by the user puts the next invocation into the silent-reset path the skill explicitly prevents — `force-restart` is the documented escape hatch precisely because manual `rm` is not. When invoked with `force-restart` AND a stale state file is present, the orchestrator logs `[coverage-expansion] force-restart authorised: deleting stale state file (<conflict summary>) and starting from Pass 1`, deletes the file, then proceeds with a fresh run. When invoked with `force-restart` AND no state file is present, the arg is a no-op (no harm done; explicit log line `force-restart: no stale state to clear`).
...
- **`coverage-expansion-state.json`** — in the per-pass record, a `dispatches` array with one entry per journey: `{ journey: "j-<slug>", result: "new-tests-landed|no-new-tests|blocked|skipped|rejected", reason: "<text or null>", authorizer: "<user|null>", rejected: <integer|null>, rejectedParts: ["trigger 1"|"trigger 2"|"trigger 3"|"mapping-table-header"|"per-expectation entries", ...] | null }`. `authorizer` is only non-null for `skipped`. `rejected` and `rejectedParts` are non-null only when `result == "rejected"` — the cap-hit case from §"Re-dispatch cap" below; `rejected` is the redispatch count at cap-hit (always 3 under the current cap), `rejectedParts` is the list of conformance items the subagent failed to produce. `result == "rejected"` is **not** a no-skip contract terminal — the pass remains incomplete until the user intervenes and resume-after-intervention finishes the journey with one of the four real terminals.
...
**Mid-pass crossing of the 70% threshold.** The default check is between passes (above), but a long-running pass with many journeys can cross 70% mid-wave. This case sits at the intersection of §"Non-negotiables for depth mode" bullet 3 (budget pressure → commit + write state + stop with resume-needed) and the auto-compaction default (between-pass seam). Resolution preserves both contracts and does not introduce any new commit type:

1. **Allow the in-flight wave to complete.** Dispatched subagents return as normal; do not kill them mid-flight. Killed subagents lose work, which §"Non-negotiables" bullet 3 explicitly forbids.
2. **Each completed journey commits per the standard convention.** Compositional passes commit per-journey per-variant; adversarial passes commit per-journey. See §"Commit-message conventions" — no new commit-message template is introduced for mid-pass crossing. The mid-pass crossing simply means the pass's full set of journey commits is split across two orchestrator sessions (pre-compact and post-compact); the git log makes this visible naturally because each journey's commit message names the journey ID.
3. **Write state with the in-flight roster populated.** `inFlightJourneys[]` lists journeys whose dispatch had not yet started or whose returns had not yet landed at threshold-cross; `completedJourneys[]` lists journeys whose commits landed in this session. The state-file shape (§"Authoritative state file") already supports this.
4. **If `/compact` is available** (or the platform-equivalent compaction primitive), auto-compact per the four-step flow above. The post-compact resume reads the state file and continues from `inFlightJourneys[]`. The pass is marked complete only when every journey in `journeyRoster[]` has either landed in `completedJourneys[]` or terminated with one of the four no-skip results.
5. **If `/compact` is NOT available**, emit the safe-compact line per the Platform note above and stop with resume-needed per §"Non-negotiables" bullet 3. The next invocation reads the state file and continues from `inFlightJourneys[]`.

Mid-pass crossing does NOT relax the no-skip contract or the per-pass completion criteria — it relaxes only the assumption that a pass is dispatch-and-commit in one continuous orchestrator session. The pass remains incomplete until the resume finishes it.
...
**Re-dispatch cap.** Each rejected return consumes one of three redispatches per journey per pass. After three consecutive rejections of the same journey, the orchestrator stops dispatching that journey and **halts the pass** with a surfaced message of the form `[coverage-expansion] re-pass conformance failure on j-<slug> after 3 redispatches: <comma-separated rejected parts>; pass <N> halted, resume after intervention`. The orchestrator does NOT synthesise a `blocked` return for this journey — `blocked` is subagent-returned per the no-skip contract, and a synthesised one would corrupt the no-skip ledger. Halt is the correct disposition: re-pass conformance failure is skill-level infra (the subagent cannot produce the contract's evidence shape from the inputs given), not test/app failure, and matches the existing rule that infra errors halt the pipeline.

The state file records the in-flight roster at halt time (`inFlightJourneys[]` includes the rejected journey; `dispatches[]` retains the **final** (third-rejection) result for that journey ID, with a `rejected: 3` flag and the rejected-parts list). Resume after user intervention proceeds from the in-flight roster. The user's intervention is one of: fix the journey block in `journey-map.md`, fix the skill, or explicitly authorise a `skipped` for that journey via in-conversation request before re-invocation.

This cap composes with the no-skip contract intentionally: a halted pass is **incomplete**, not "complete with one journey skipped." The pass cannot be marked complete until every journey has a non-rejected terminal state. The cap simply prevents indefinite looping; it does not relax the no-skip contract's completeness requirement.
