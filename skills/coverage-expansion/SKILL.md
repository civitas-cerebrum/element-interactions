...
2. **Every journey in the map gets a dispatch every adversarial pass.** Pass 4 and Pass 5 run bug-discovery per journey — 0 journeys × Pass 4 is not Pass 4. A journey whose adversarial subagent returns "no meaningful boundaries found" must still be recorded in the ledger section with that result — the dispatch happened.
   ...
   **Pass-4 prelude — portal-wide pattern scan.** Before the per-journey Pass-4 dispatches start, the orchestrator dispatches **one** `probe-portal-wide:` subagent that establishes the portal-wide pattern catalogue at `tests/e2e/docs/portal-wide-patterns.md`. The catalogue documents recurring patterns (CSRF tamper status, autocomplete attrs, sort-unknown-field handling, response headers, error envelope shapes, asset disclosure, rate limiting, session-cookie flags) that ~80% of per-journey `info`-severity findings empirically duplicate. After the scan, every per-journey Pass-4 / Pass-5 dispatch receives the catalogue file as input and cites patterns via `coverage: portal-wide:<pattern-id>` rather than re-deriving them. Full spec: [`references/portal-wide-scan.md`](references/portal-wide-scan.md). The scan does NOT run in `mode: breadth`.
...
- **P3 small-surface journeys may opt OUT of adversarial passes (Passes 4 & 5).** P3 logout / role-chooser / modal-only journeys empirically produce 0–2 unique adversarial findings each — their entire adversarial surface is already covered via portal-wide pattern citations from larger journeys. The skip is **opt-in per project**, declared up-front in the state file's `adversarialSkippedJourneys: []` field with rationale per entry. Default is "include all" — the orchestrator never silently skips a P3 from adversarial work; the operator opts the journey out by name. Compositional passes (1–3) ALWAYS run on every journey including P3. **Exclusion criteria** (must hold for a journey to qualify for opt-out):
  - Priority is P3.
  - The journey's `Pages touched` list is a subset of pages already adversarial-probed by a larger journey AND covered by a portal-wide pattern entry.
...
- **The orchestrator does NOT compose tests directly.** Spec writes for `tests/e2e/j-<slug>.spec.ts` and `tests/e2e/sj-<slug>.spec.ts` (and their `-regression` variants) come from a dispatched `composer-j-<slug>:` / `composer-sj-<slug>:` / `probe-j-<slug>:` subagent — never from direct orchestrator action. **Harness-enforced as a hard DENY** by `hooks/coverage-expansion-direct-compose-block.sh` (PostToolUse:Write|Edit). The dispatch-guard (`hooks/coverage-expansion-dispatch-guard.sh`) registers each `composer-j-<slug>:` / `composer-sj-<slug>:` / `probe-j-<slug>:` dispatch in `tests/e2e/docs/.in-flight-composers.json` with a 30-min TTL; the block-hook ALLOWS spec writes whose slug matches an in-flight entry (legitimate composer-subagent work) and DENIES otherwise (orchestrator-direct composition). `tests/e2e/happy-path.spec.ts` is exempt (Phase 3 of onboarding writes it before coverage-expansion's Pass 1).

  **Handover envelope (shorter leash than TTL).** The 30-min TTL is a failsafe for crashed / abandoned dispatches. The primary cleanup path is the §2.0 handover envelope in [`../element-interactions/references/subagent-return-schema.md`](../element-interactions/references/subagent-return-schema.md): every composer / reviewer / probe / process-validator / phase-validator return MUST be prefaced with `handover: { role, cycle, status, next-action }`. The PostToolUse return-schema guard (`hooks/subagent-return-schema-guard.sh`) reads the envelope and, for composer + probe slugs, **deregisters the slot immediately on terminal status** — the registry slot clears as soon as the subagent hands over, not 30 minutes later. Cycle-mismatch (envelope claims `cycle: 1` against a registry entry at `cycle: 2`) refuses to deregister and emits a fix-message. Reviewer / process-validator / phase-validator handovers carry the envelope for orchestrator-side `next-action` routing, but those roles are not in the registry, so their envelopes have no harness-side registry effect (the composer slot they correspond to is held / refreshed via the orchestrator's redispatch under the same composer-j-<slug>). Initial release ships envelope-validation in WARN mode; deregistration itself fires regardless of mode (it's bookkeeping, not validation). See [`../element-interactions/references/subagent-return-schema.md`](../element-interactions/references/subagent-return-schema.md) §2.0 + §4.3.

  **Reviewer spillover contract (hard enforcement, scoped to `improvements-needed`).** A reviewer's `improvements-needed` return has historically carried the full `missing-scenarios:` / `craft-issues:` / `verification-misses:` sub-lists with all sub-bullets in the body — typically 1-3k tokens of structured detail absorbed into the orchestrator's transcript every cycle. Per [`../element-interactions/references/subagent-return-schema.md`](../element-interactions/references/subagent-return-schema.md) §2.6, the reviewer writes that detail to a canonical spill file at `tests/e2e/docs/.subagent-returns/reviewer-<journey>-<pass>-c<cycle>.md` and emits only the index-level fields (`status`, `journey`, `pass`, `cycle`, `spill:` pointer, `findings:` ID list) inline. **Hard-enforced via `hooks/subagent-spillover-rewrite-gate.sh`** — a `SubagentStop` hook that intercepts non-compliant returns before they reach the orchestrator's transcript: exit 2 with stderr feedback blocks the subagent's stop, the feedback injects as next-turn input, the subagent rewrites in-session, and the orchestrator's tool result is the FINAL compliant return. Per-`agent_id` rewrite counter caps at 3 → exit 0 with `[CAP-REACHED]` WARN if compliance is unreachable. The `PostToolUse:Agent` schema-guard adds a defense-in-depth WARN when a spill file is absent — rare in practice (the enforcer prevents it) but covers mis-installed-hook edge cases. The orchestrator constructs the next composer-cycle brief by referencing the spill-file path; it MUST NOT inline the spill contents (that would defeat the purpose). Composer briefs / probe briefs / phase-validator briefs are not yet under spillover — extension follows after this initial rollout produces real-world compliance data.
- **The orchestrator does NOT run `playwright-cli` for selector inspection.** The CLI session belongs to the dispatched subagent (its slug carries the subagent's role prefix — see §"Role prefixes"). Orchestrator-side `playwright-cli` use during coverage-expansion is a discipline violation: it pulls DOM snapshots into the orchestrator's context. **Harness-enforced as a hard DENY** by `hooks/coverage-expansion-orchestrator-cli-block.sh` (PreToolUse:Bash) — when `tests/e2e/docs/coverage-expansion-state.json` is present and a `playwright-cli -s=<slug>` invocation's slug doesn't map to any registered in-flight composer/probe entry, the call is denied with a redirect to dispatch a `stage2-<scope>:` / `probe-j-<slug>:` / `composer-j-<slug>:` subagent. Session-agnostic subcommands (`close-all`, `kill-all`, `list`, `install-browser`, `--version`, `--help`) are exempt — the orchestrator legitimately runs `close-all` at pass exit. Reviewer / phase1 / stage2 / cleanup subagents' CLI sessions also pass (they're dispatched through the Agent-tool path, validated upstream).
