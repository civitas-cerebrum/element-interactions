...
2. **Every journey in the map gets a dispatch every adversarial pass.** Pass 4 and Pass 5 run bug-discovery per journey — 0 journeys × Pass 4 is not Pass 4. A journey whose adversarial subagent returns "no meaningful boundaries found" must still be recorded in the ledger section with that result — the dispatch happened.
   ...
   **Pass-4 prelude — portal-wide pattern scan (issue #164.3).** Before the per-journey Pass-4 dispatches start, the orchestrator dispatches **one** `probe-portal-wide:` subagent that establishes the portal-wide pattern catalogue at `tests/e2e/docs/portal-wide-patterns.md`. The catalogue documents recurring patterns (CSRF tamper status, autocomplete attrs, sort-unknown-field handling, response headers, error envelope shapes, asset disclosure, rate limiting, session-cookie flags) that ~80% of per-journey `info`-severity findings empirically duplicate. After the scan, every per-journey Pass-4 / Pass-5 dispatch receives the catalogue file as input and cites patterns via `coverage: portal-wide:<pattern-id>` rather than re-deriving them in their own ledger sections. Full spec: [`references/portal-wide-scan.md`](references/portal-wide-scan.md). The scan does NOT run in `mode: breadth`.
