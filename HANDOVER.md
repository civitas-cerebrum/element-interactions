...
4. **~~`@playwright/cli` is a soft dep for now.~~** **Superseded 2026-05-01 — promoted to a hard `dependencies` entry, pinned to `0.1.10`.** The "install hint" UX was rejected by the project owner: "installing this package should automatically install playwright cli tool." Trade-off accepted: alpha-version pin tightens (caret → exact) on every CLI minor bump; this package's release absorbs each CLI breaking change so consumers never have to think about CLI versions. The pin loosens to `^X.Y.Z` only after the CLI clears 1.0.
