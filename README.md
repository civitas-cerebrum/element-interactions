...
* **`dropFiles(elementName, pageName, filenames: string[], options?: { mimeType? })`** — Simulates dropping files onto a drop-zone by dispatching `dragenter`, `dragover`, and `drop` events with a populated `DataTransfer`. Filenames are basenames only — no real file is read from disk.
