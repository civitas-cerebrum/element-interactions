...
- **Cookie name:** `<auth-cookie-name>`  *(if cookie-based)*
- **Login endpoint:** `POST <login-path>` — body `<credential payload shape>`, 200 sets cookie
- **Signup endpoint:** `POST <signup-path>` — body `<signup payload shape>`, 200 sets cookie + auto-authenticates  *(omit if app has no public signup)*
- **Logout endpoint:** `POST <logout-path>` — clears cookie

### Reset / seed endpoints

- **Reset:** `POST <reset-path>` — 200, no body, resets DB to seed state
- **Seed-only:** `POST <seed-path>` — 200  *(omit if not separate)*
...
- **<banner-or-modal-label>:** `<container-selector>`, dismissed via `<dismiss-selector>`. *(One bullet per observed banner / modal. Note when each appears — first visit per origin, first signup, etc.)*

*(Or: "none discovered.")*

### Stable seed resources

- **<primary-resource-collection>:** N stable IDs `<resource-001>` … `<resource-N>` observed at first render of the relevant collection page.
- **<secondary-resource-collection>:** M stable seed records observed on `<collection-path>` first render, owned by `<seed-owner-id>`.  *(omit collections that don't have stable seeds)*
...
|---|---|---|---|---|
| POST | `<login-path>` | <auth page> submit | `<credential payload shape>` | 200 + Set-Cookie |
| POST | `<signup-path>` | <signup page> submit | `<signup payload shape>` | 200 + Set-Cookie |
| POST | `<resource-create-path>` | "<action button label>" | `<creation payload shape>` | 200 |
| PUT  | `<resource-update-path>/:id` | <field> input change | `<update payload shape>` | 200 |
| POST | `<resource-action-path>/:id/<verb>` | "<action button label>" | `<action payload shape>` | 200 |
