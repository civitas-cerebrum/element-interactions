...
**Subagent returns** are validated against per-role JSON Schemas under [`methodology/schemas/subagent-returns/`](../../../schemas/subagent-returns/README.md). That directory is the source of truth — see its README for the role-to-schema table and authoring conventions.
...
Full schema: `methodology/schemas/subagent-returns/handover.schema.json`.
...
Full schema: `methodology/schemas/subagent-returns/phase-validator.schema.json`.
...
1. Link to this file in its SKILL.md, using the relative path `methodology/skills/element-interactions/references/subagent-return-schema.md`.
...
- Transport format for finding blocks. The §1 finding-return format is prose/Markdown. The §2.x handover-envelope returns use JSON (preferred) or YAML — per-role schemas live in `methodology/schemas/subagent-returns/`.
