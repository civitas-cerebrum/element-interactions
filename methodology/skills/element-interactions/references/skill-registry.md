...
**SKILL.md path convention.** Every skill's main doc lives at `methodology/skills/<skill-name>/SKILL.md` — slug the Invocation string to get the path. Reference docs live under `methodology/skills/<skill-name>/references/`. For example, `coverage-expansion` → `methodology/skills/coverage-expansion/SKILL.md`.
