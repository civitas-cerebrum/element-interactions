...
- `onboarding-ledger-write-gate.sh` (PreToolUse:Write|Edit, DENY) —
  ...
  `methodology/schemas/onboarding-status.schema.json` and denies phase-skip
...
- `methodology/schemas/onboarding-status.schema.json` — ledger shape (v1)
- `methodology/schemas/subagent-returns/workflow-reviewer.schema.json` — reviewer return shape
- `methodology/skills/workflow-reviewer/SKILL.md` — reviewer methodology
- `methodology/skills/element-interactions/references/harness-hooks.md` — both new hooks indexed
...
3. The composer skill internally runs an in-loop reviewer pass that
   ...
   `reviewer-inloop` schema (see `methodology/schemas/subagent-returns/`). You
...
`methodology/schemas/subagent-returns/composer.schema.json` and
...
>   (`Skill('journey-mapping')` or `Read` of `methodology/skills/journey-mapping/
...
- **Return-shape conformance.** Every subagent dispatch you run must
  ...
  `methodology/schemas/subagent-returns/`).
