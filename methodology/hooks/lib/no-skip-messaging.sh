...
# Rationale
# ---------
# An earlier sandbox-bypass post-mortem identified that several
...
#   4. The pointer to methodology/skills/onboarding/SKILL.md §"Hard rules — kernel-
#      resident".
#
...
# Canonical reference
# -------------------
# methodology/skills/onboarding/SKILL.md §"Hard rules — kernel-resident"
#
# Usage
# -----
...
Reference: methodology/skills/onboarding/SKILL.md §"Hard rules — kernel-resident"
NO_SKIP_BLOCK_EOF
}

# --- self-test ----------------------------------------------------------------
# Run as `NO_SKIP_MESSAGING_SELFTEST=1 bash methodology/hooks/lib/no-skip-messaging.sh`.
...
if [ "${NO_SKIP_MESSAGING_SELFTEST:-0}" = "1" ]; then
  ...
  for substr in \
      ...
      "NOT authorisation" \
      "methodology/skills/onboarding/SKILL.md"; do
