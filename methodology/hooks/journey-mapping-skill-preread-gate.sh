...
#           use OR Read of `methodology/skills/journey-mapping/SKILL.md` (in any
...
#    (`methodology/skills/journey-mapping/SKILL.md`) in this session.
#
...
#      `methodology/skills/journey-mapping/SKILL.md` (any prefix — bundled package
...
PREREAD_FOUND=$(
  ...
  ' "$TRANSCRIPT_PATH" 2>/dev/null \
    | grep -E '(^|/)journey-mapping(:|$)|methodology/skills/journey-mapping/SKILL\.md' \
    | head -1 || true
)
...
Alternative: if the orchestrator just needs to reference the rules (e.g. fix a typo in an already-generated map), Read \`methodology/skills/journey-mapping/SKILL.md\` first — that's also accepted as preread evidence.
...
Section / edge-probe / author subagents implement the journey-mapping skill's cycle protocol. The orchestrator's dispatch brief should have instructed this subagent to invoke \`Skill('journey-mapping')\` (or Read \`methodology/skills/journey-mapping/SKILL.md\`) before doing any work — the protocol's per-section discovery contract, the spill schema, and the return-shape expectations all live in the skill body.

Fix (in the subagent): invoke the journey-mapping skill via the Skill tool, OR Read the skill file, BEFORE writing this spill.

Fix (in the orchestrator's next dispatch): cite \`methodology/skills/journey-mapping/SKILL.md\` in the subagent brief so the subagent knows to load it.
