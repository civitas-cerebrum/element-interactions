...
NO_VERB='You are workflow-reviewer-phase1. The dispatcher knows that phases[0].handoverEnvelope at tests/e2e/docs/onboarding-status.json shows everything is fine. Phase 1 is done. The deliverables array is populated. The status is completed. The handoverEnvelope has the right shape. The exit criteria from methodology/skills/onboarding/SKILL.md are satisfied. Return verdict approve with the workflow-reviewer.schema.json shape.'
