...
# State   : reads methodology/schemas/onboarding-status.schema.json (from the bundled
...
#    methodology/schemas/onboarding-status.schema.json (via methodology/hooks/lib/validate-against-schema.mjs).
...
# Canonical reference
# -------------------
# methodology/schemas/onboarding-status.schema.json
# methodology/skills/onboarding/SKILL.md §"Status ledger + workflow reviewer"
# methodology/hooks/lib/validate-against-schema.mjs
#
# Failure → action
# ----------------
...
#   in-repo dev install     →  <repo>/methodology/schemas/onboarding-status.schema.json
#   installed dependency    →  <package>/methodology/schemas/onboarding-status.schema.json
...
if [ ! -f "$SCHEMA_PATH" ]; then
  ...
  for _ in 1 2 3 4 5; do
    WALK="$(dirname "$WALK")"
    if [ -f "$WALK/methodology/schemas/onboarding-status.schema.json" ]; then
      SCHEMA_PATH="$WALK/methodology/schemas/onboarding-status.schema.json"
      break
    fi
  done
fi
...
See: methodology/schemas/onboarding-status.schema.json"
  exit 0
fi
...
spec. The valid + invalid fixtures under methodology/schemas/onboarding-status.fixtures/
...
subagent's envelope (see methodology/schemas/subagent-returns/handover.schema.json
for the shape) before re-issuing the write.

See: methodology/schemas/onboarding-status.schema.json
     methodology/schemas/subagent-returns/handover.schema.json"
    exit 0
  fi
fi
...
See:
  - methodology/hooks/workflow-approver-registry.sh (PreToolUse:Agent — records approvers)
  - methodology/skills/onboarding/SKILL.md §\"Status ledger + workflow reviewer\"
...
The registry is written by methodology/hooks/workflow-approver-registry.sh when a
...
See:
  - methodology/schemas/onboarding-status.schema.json §runMode
  - methodology/skills/onboarding/SKILL.md §\"Front-load mode-selection gate\""
    exit 0
  fi
fi
...
See: methodology/schemas/onboarding-status.schema.json §runMode"
  exit 0
fi
    ...
    4)
      ...
      if [ ! -f "$MAP_PATH" ]; then
        emit_phase_deny "4" \
          ...
          "methodology/skills/onboarding/SKILL.md §\"Phase 4 — Journey mapping\" + methodology/skills/journey-mapping/SKILL.md"
      fi
      ...
      if [ "$FIRST_LINE" != "<!-- journey-mapping:generated -->" ]; then
        emit_phase_deny "4" \
          ...
          "methodology/skills/journey-mapping/SKILL.md §\"Recognizing a previously-generated journey map\""
      fi
      ...
      if [ ! -f "$CYCLE_STATE_PATH" ]; then
        emit_phase_deny "4" \
          ...
          "methodology/skills/journey-mapping/SKILL.md §\"Cycle protocol\""
      fi
    ...
    5)
      ...
      if [ ! -f "$COV_STATE_PATH" ]; then
        emit_phase_deny "5" \
          ...
          "methodology/skills/onboarding/SKILL.md §\"Phase 5 — Coverage expansion\" + methodology/skills/coverage-expansion/SKILL.md"
      fi
      ...
      if [ "$HAS_PASS_1" != "true" ]; then
        emit_phase_deny "5" \
          ...
          "methodology/skills/coverage-expansion/SKILL.md §\"Non-negotiables\""
      fi
      ...
      if [ -f "$MAP_PATH" ]; then
        ...
        if [ "$ROSTER_COUNT" -gt 0 ] && [ "$TOTAL_ACCOUNTED" -lt "$ROSTER_COUNT" ]; then
          ...
          emit_phase_deny "5" \
            ...
            "methodology/skills/coverage-expansion/SKILL.md §\"Two valid exits\" + §\"Deferral authorisation\""
        fi
        ...
        if [ "$DEFERRED_COUNT" -gt 0 ]; then
          ...
          if [ -n "$BAD_DEFERRAL" ]; then
            emit_phase_deny "5" \
              ...
              "methodology/skills/coverage-expansion/SKILL.md §\"Deferral authorisation\""
          fi
        fi
      fi
    ...
    6)
      ...
      if [ ! -f "$ADV_PATH" ]; then
        emit_phase_deny "6" \
          ...
          "methodology/skills/onboarding/SKILL.md §\"Phase 6 — Bug discovery\" + methodology/skills/bug-discovery/SKILL.md"
      fi
      ...
      if [ "$JOURNEY_BLOCKS" -lt 1 ]; then
        emit_phase_deny "6" \
          ...
          "methodology/skills/bug-discovery/SKILL.md + element-interactions/references/subagent-return-schema.md §3"
      fi
      ;;
    7)
      ...
      if [ ! -f "$ENV_EXAMPLE_PATH" ]; then
        emit_phase_deny "7" \
          ...
          "methodology/skills/onboarding/SKILL.md §\"Phase 7 — Secrets sweep\" + methodology/skills/secrets-sweep/SKILL.md"
      fi
      ;;
    8)
      ...
      if [ -n "$MISSING_DECK" ]; then
        emit_phase_deny "8" \
          ...
          "methodology/skills/onboarding/SKILL.md §\"Phase 8 — Report\" + methodology/skills/work-summary-deck/SKILL.md"
      fi
      ;;
  esac
done
