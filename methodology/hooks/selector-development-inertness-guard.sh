...
# is installed into ~/.claude/hooks/ (where $ws/methodology/hooks/lib won't exist).
...
# Fallback for dev-repo edge case where HOOK_DIR/lib doesn't exist but $ws/methodology/hooks/lib does
# (e.g. when testing the hook directly from the repo root without postinstall).
[ -d "$HOOK_LIB" ] || HOOK_LIB="$ws/methodology/hooks/lib"
