...
- The script `methodology/scripts/validate-schema-fixtures.mjs` exercises every fixture against every schema.
- The hook `methodology/hooks/subagent-return-schema-guard.sh` validates live subagent returns against the same schemas at runtime via `methodology/hooks/lib/validate-against-schema.mjs`.
...
3. Run `node methodology/scripts/validate-schema-fixtures.mjs` and confirm both fixtures behave as expected.
